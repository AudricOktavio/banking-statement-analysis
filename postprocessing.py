...
def reformat_dataframe(df):
    """
    Reformats a DataFrame by setting the row with 'TANGGAL' or 'TANGGAL TRANSAKSI' (case-insensitive, exact match)
    as the column headers and extracting the table data starting from the row below it. If no such row is found,
    ...
    Returns:
    pd.DataFrame: The reformatted DataFrame or original DataFrame if 'TANGGAL' or 'TANGGAL TRANSAKSI' is not found.
    """
    # Step 1: Find the row index where any cell matches 'TANGGAL' or 'TANGGAL TRANSAKSI' exactly (case-insensitive)
    tanggal_row_idx = df.apply(
        lambda row: row.str.strip().str.lower().isin(["tanggal", "tanggal transaksi"]).any(), axis=1
    ).idxmax()

    # Step 2: Check if the row contains 'TANGGAL' or 'TANGGAL TRANSAKSI'
    if df.iloc[tanggal_row_idx].str.strip().str.lower().isin(["tanggal", "tanggal transaksi"]).any():
        # Step 3: Set the identified row as the new column names
        df.columns = df.iloc[tanggal_row_idx].fillna("").str.strip()
